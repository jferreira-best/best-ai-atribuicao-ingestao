...
- conhecimento        : "Atribuição de Classes (AC)" | "Avaliação de Desempenho (AD)" | "Geral"
- norma_tipo          : "Portaria" | "Resolução" | "Comunicado" | "Informação" | ...
- orgao_emissor       : "SEDUC" | "DIPES" | "SUCOR" | "SUPED" | ...
- data_publicacao     : "YYYY-MM-DD" (se detectável)
- ano_letivo          : "2025" | "2026" | ...
- fase_processo       : "Confirmação de Participação – Fase 1", "Classificação – Fase 2", ...
- programa            : "PEI" | "Tempo Integral" | "Tempo Parcial" | ...
- publico_alvo        : "Docentes", "Gestores", "PEI", etc.
- prazo_inicio/fim    : datas (YYYY-MM-DD) se algum intervalo explícito for detectado
- referencias_legais  : lista de atos normativos citados (ex.: "Resolução SEDUC nº 132/2025")

Mantém a estrutura do script original (CLI, leitura de PDF/DOCX/TXT, chunking e JSONL),
apenas enriquecendo cada registro com campos adicionais.
...
DOE_CODE_RE = re.compile(r"\b20\d{2}\.\d{2}\.\d{2}\.[\d.]+\b")  # ex. 2025.10.13.1.2.18....
...
def guess_fase_processo(text: str, fname: str) -> str:
    ...
    if "confirmação de participação" in s_low or "confirmacao de participacao" in s_low:
        # tenta capturar "Fase X"
        m = re.search(r"fase\s*\d", s_low)
        return f"Confirmação de Participação – {m.group(0).title()}" if m else "Confirmação de Participação"
    if "classificação" in s_low or "classificacao" in s_low:
        m = re.search(r"fase\s*\d", s_low)
        return f"Classificação – {m.group(0).title()}" if m else "Classificação"
    return ""

def guess_programa(text: str) -> str:
    s = (text or "").lower()
    if "programa de ensino integral" in s or "pei" in s:
        return "PEI"
    if "tempo integral" in s:
...
def guess_publico_alvo(text: str) -> str:
    s = (text or "").lower()
    if "docente" in s or "docentes" in s:
        return "Docentes"
    if "gestor" in s or "diretor" in s or "coordenador" in s:
        return "Gestores"
    if "pei" in s:
        return "PEI"
    return ""
...
def guess_data_publicacao(text: str) -> str:
    ...
    # alguns AC trazem "código de autenticidade" com data embutida, mas mantemos simples
    return ""

def extract_referencias_legais(text: str) -> List[str]:
    refs = set()
    # Resolução SEDUC nº 132/2025; Resolução SE nº 83/2025 ...
    for m in re.finditer(r"(Resolu\w+.*?)\b(\d{2,4})\/(20[2-4]\d)", text, flags=re.I):
        chunk = m.group(0).strip()
        if 6 <= len(chunk) <= 140:
            refs.add(chunk)
    # Portaria nº XXXX/AAAA
    for m in re.finditer(r"(Portaria.*?)\b(\d{2,4})\/(20[2-4]\d)", text, flags=re.I):
        chunk = m.group(0).strip()
        if 6 <= len(chunk) <= 140:
            refs.add(chunk)
    return sorted(refs)
...
def main():
    ...
    ap.add_argument("--assunto", default=os.getenv("ASSUNTO", "obras"), help="Campo 'assunto' a gravar no JSONL.")
